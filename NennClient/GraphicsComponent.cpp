...
GraphicsComponent::GraphicsComponent(const GameClient* client)
	: ClientComponent(client), _device(nullptr), _driver(nullptr), _sceneManager(nullptr), _gui(nullptr), _terrain(nullptr)
{
	SIrrlichtCreationParameters params;
	params.AntiAlias = 8;
	params.Fullscreen = false;
	params.DriverType = EDT_OPENGL;
	_device = createDevice(video::EDT_OPENGL, dimension2d<u32>(640, 480), 24, false, false, true, 0);
	if (!_device)
		return;
